...
export default function DashboardCom() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/user/getusers?limit=5`);
    ...
    const fetchPosts = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/post/getposts?limit=5`);
        if (response.data.success) {
    ...
    const fetchComments = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/comment/getcomments?limit=5`);
        if (response.data.success) {
